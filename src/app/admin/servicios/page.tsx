...
export default function AdminServicios() {
  ...
  const handleDelete = async (item: Servicio) => {
    if (!(await confirmDelete(`¿Eliminar el servicio "${item.nombre}"?`))) return;
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        {loading ? (
          ...
          <p className="text-center text-slate-400 py-16">No hay servicios registrados.</p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-medium text-slate-600">Ícono</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {items.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-slate-500">{item.icono ?? "—"}</td>
                    <td className="px-4 py-3 text-slate-500">{item.orden ?? "—"}</td>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    value={form.icono}
                    onChange={handleChange}
                    placeholder="ej: heart, stethoscope"
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
                  />
                </div>
                <div>
